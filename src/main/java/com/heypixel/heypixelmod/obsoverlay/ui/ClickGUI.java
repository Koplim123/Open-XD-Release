        ...
        if (this.renderValues != null) {
            ...
            for (Value valuex : this.renderValues) {
                if (valuex.isVisible() && valuex.getValueType() == ValueType.FLOAT) {
                    ...
                    CustomTextRenderer floatFont = Fonts.opensans;
                    float floatScale = 0.4F;
                    if (StringUtils.containChinese(valuex.getName())) {
                        floatFont = Fonts.harmony;
                        floatScale = 0.325F;
                    }
            ...
            for (Value valuexx : this.renderValues) {
                if (valuexx.isVisible() && valuexx.getValueType() == ValueType.MODE) {
                    ...
                    for (int modeIndex = 0; modeIndex < modeValue.getValues().length; modeIndex++) {
                        ...
                        CustomTextRenderer modeFont = Fonts.opensans;
                    float modeScale = 0.4F;
                    if (StringUtils.containChinese(mode)) {
                        modeFont = Fonts.harmony;
                        modeScale = 0.325F;
                    }
                    modeFont.render(stack, mode, (double)(windowX + 152.0F + x), (double)(windowY + valueHeight + motion + 25.0F), Color.WHITE, true, modeScale);
                        x += currentLengthx;
                    }
